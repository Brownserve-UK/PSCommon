...
Describe 'Documentation' {
    Context 'Markdown Files' {
        ...
        It 'should have a markdown file for each cmdlet' {
            $CmdletNames = Get-Command -Module Brownserve.PSCommon | Select-Object -ExpandProperty Name
            $CmdletNames | ForEach-Object {
                $CmdletName = $_
                $CmdletPath = Join-Path -Path $Global:BrownserveRepoDocsDirectory -ChildPath "Brownserve.PSCommon" -AdditionalChildPath "$CmdletName.md"
                Test-Path -Path $CmdletPath | Should -Be $true
            }
        }
        It 'should not have any incomplete sections in markdown files' {
            $MarkdownFiles = Get-ChildItem -Path (Join-Path $Global:BrownserveRepoDocsDirectory -ChildPath "Brownserve.PSCommon") -Filter *.md -Recurse
