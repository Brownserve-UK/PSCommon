...
BeforeAll {
    Remove-Module Brownserve.PSCommon -ErrorAction SilentlyContinue -Verbose:$false
    Join-Path $global:BrownserveBuiltModuleDirectory 'Brownserve.PSCommon.psd1' | Import-Module -Force -Verbose:$false
}

Describe 'ModuleImport' {
    Context 'When Brownserve.PSCommon is imported' {
    ...
    Context 'Exported cmdlets' {
        BeforeAll {
            ...
            $script:ExportedCmdlets = Get-Command -Module 'Brownserve.PSCommon' |
